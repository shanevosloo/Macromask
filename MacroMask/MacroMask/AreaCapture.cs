    ...
    public partial class AreaCapture : Form
    {
        ...
        public void SaveSelection(bool showCursor)
        {
            ...
            if (ScreenPath != "" || ScreenShot.saveToClipboard)
            {
                ...
                if (bounds.Height == 0 || bounds.Width == 0) bounds = Screen.GetBounds(Screen.GetBounds(Point.Empty));
                string fi = "";
                ...
                if (ScreenShot.saveToClipboard)
                {
                    MessageBox.Show("Area saved to clipboard", "MacroMask", MessageBoxButtons.OK);
                }
                else
                {
                    MessageBox.Show("Area saved to file", "MacroMask", MessageBoxButtons.OK);
                }
            ...
            {
                MessageBox.Show("File save cancelled", "MacroMask", MessageBoxButtons.OK);
                this.InstanceRef.Show();
                this.Close();
            }
        }
        ...
        private void mouse_DClick(object sender, MouseEventArgs e)
        {
            ...
            {
                ScreenShot.selectionRect = new Rectangle(CurrentTopLeft.X, CurrentTopLeft.Y, CurrentBottomRight.X - CurrentTopLeft.X, CurrentBottomRight.Y - CurrentTopLeft.Y);
                if (!ScreenShot.selectionOnly)
                {
                    SaveSelection(false);
                }
                else
